...
int main()
{
    ...
    while (1)
    {
        ...
        if (!state.EX.nop)
        {
            ...
            bitset<32> op2_reg = state.EX.Read_data2; 

    
            bool forward_mem_rs = (!state.MEM.nop && state.MEM.wrt_enable && state.MEM.Wrt_reg_addr.to_ulong() != 0 && state.MEM.Wrt_reg_addr == state.EX.Rs);
            bool forward_mem_rt = (!state.MEM.nop && state.MEM.wrt_enable && state.MEM.Wrt_reg_addr.to_ulong() != 0 && state.MEM.Wrt_reg_addr == state.EX.Rt);

            if (forward_mem_rs)
                op1 = state.MEM.ALUresult;
            if (forward_mem_rt)
                op2_reg = state.MEM.ALUresult;

            bool forward_wb_rs = (!state.WB.nop && state.WB.wrt_enable && state.WB.Wrt_reg_addr.to_ulong() != 0 && state.WB.Wrt_reg_addr == state.EX.Rs);
            bool forward_wb_rt = (!state.WB.nop && state.WB.wrt_enable && state.WB.Wrt_reg_addr.to_ulong() != 0 && state.WB.Wrt_reg_addr == state.EX.Rt);

            if (forward_wb_rs && !forward_mem_rs)
                op1 = state.WB.Wrt_data;
            if (forward_wb_rt && !forward_mem_rt)
                op2_reg = state.WB.Wrt_data;
            ...
            newState.MEM.Store_data = op2_reg; 
        ...
        if (!state.ID.nop)
        {
            ...
            else
            {
                ...
                if (is_branch && (newState.EX.Read_data1 != newState.EX.Read_data2))
                {
                    ...
                    bitset<32> offset;
                    if (b_imm[0] == '1')
                        offset = bitset<32>(string(19, '1') + b_imm);
                    else
                        offset = bitset<32>(string(19, '0') + b_imm);
        ...
        if (!state.EX.nop && state.EX.rd_mem)
        {
            ...
            if (!state.ID.nop && state.ID.Instr.to_ulong() != 0xffffffff)
            {
                string instr_str = state.ID.Instr.to_string();
                string opcode = instr_str.substr(25, 7);
                bitset<5> id_rs1(instr_str.substr(12, 5));
                bitset<5> id_rs2(instr_str.substr(7, 5));

                
                bool uses_rs1 = (opcode != "1101111");                                               
                bool uses_rs2 = (opcode == "0110011" || opcode == "0100011" || opcode == "1100011"); 

            
                if (state.EX.Wrt_reg_addr.to_ulong() != 0)
                {
                    if (uses_rs1 && state.EX.Wrt_reg_addr == id_rs1)
                        is_load_use_hazard = true;
                    if (uses_rs2 && state.EX.Wrt_reg_addr == id_rs2)
                        is_load_use_hazard = true;
                }
            }
        }
