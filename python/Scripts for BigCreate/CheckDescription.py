...
def RT_trace_ray(scene, ray_orig, ray_dir, lights, depth=0):
    ...
    for light in lights:
        ...
        new_orig = hit_loc + hit_norm * eps # REPLACE WITH YOUR CODE
        #
        ...
        I_diffuser = np.array(diffuse_color) * I_light * (max(light_dir.dot(hit_norm), 0))

        color += I_diffuser # REPLACE WITH YOUR CODE
        #
        ...
        half_vec = (-ray_dir + light_dir).normalized()

        I_specular = np.array(specular_color) * I_light * (hit_norm.dot(half_vec)) ** specular_hardness
    ...
    if no_light_hit:
        I_ambient = np.array(diffuse_color) * np.array(ambient_color) # REPLACE WITH YOUR CODE
    ...
    if mat.use_fresnel:
        ...
        theta = hit_norm.dot(-ray_dir)
        
        reflectivity = R_0 + (1 - R_0) * (1 - theta) ** 5 # REPLACE WITH YOUR CODE
    #
    ...
    if depth > 0:
        ...
        D_reflect = D_in - 2 * (D_in.dot(hit_norm)) * hit_norm


        # Recursively trace the reflected ray and store the return value as a color L_reflect
        reflect_color = RT_trace_ray(scene, new_orig, D_reflect, lights, depth - 1) # REPLACE WITH YOUR CODE
        ...
        if mat.transmission > 0:
            # FILL IN YOUR CODE
            tran_orig = hit_loc - hit_norm * eps

            rev_hit_norm = -hit_norm

            cos_theta1 = min(hit_norm.dot(-ray_dir), 1.0)

            sin_theta1 = (1 - cos_theta1 ** 2) ** 0.5

            vec_help = (ray_dir + hit_norm * cos_theta1).normalized()

            if not ray_inside_object:
                n1, n2 = 1, mat.ior
            ...
            if (1 - sin_theta2 ** 2) > 0:
                ...
                new_dir = rev_hit_norm * cos_theta2 + vec_help * sin_theta2

                L_transmit = RT_trace_ray(scene, tran_orig, new_dir, lights, depth - 1)

                # Add transmission to the final color: (1 - k_r) * L_transmit
                color += L_transmit * (1 - reflectivity) # REPLACE WITH YOUR CODE
    #
